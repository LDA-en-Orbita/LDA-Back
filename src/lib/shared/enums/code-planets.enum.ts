...
export function isValidCode(code: unknown): code is CodePlanetsEnums {
  return typeof code === "string" && (PLANET_CODES as string[]).includes(code);
}

export function assertValidCode(code: unknown): asserts code is CodePlanetsEnums {
  if (!isValidCode(code)) {
    throw new Error(
...
export function listPlanets(
  ...
  return PLANET_CODES.map((c) => ({ code: c as CodePlanetsEnums, name: CODE_TO_NAME[locale][c as CodePlanetsEnums] }));
}
